    ...
    AccessControlUpgradeable
{
    ...
    // ************************************** DATA STRUCTURE **************************************
    /*
    Basically, any point in time, the amount of KKs entitled to a user but is pending to be distributed is:

    pending KK = (user.stAmount * pool.accKKPerST) - user.finishedKK

    Whenever a user deposits or withdraws staking tokens to a pool. Here's what happens:
    1. The pool's `accKKPerST` (and `lastRewardBlock`) gets updated.
    2. User receives the pending KK sent to his/her address.
    3. User's `stAmount` gets updated.
    4. User's `finishedKK` gets updated.
    */
    struct Pool {
        ...
        // Weight of pool
        uint256 poolWeight;
        // Last block number that KKs distribution occurs for pool
        uint256 lastRewardBlock;
        // Accumulated KKs per staking token of pool
        uint256 accKKPerST;
    ...
    struct User {
        ...
        uint256 stAmount;
        // Finished distributed KKs to user
        uint256 finishedKK;
        // Pending to claim KKs
        uint256 pendingKK;
    ...
    // ************************************** STATE VARIABLES **************************************
    // First block that KKStake will start from
    uint256 public startBlock;
    // First block that KKStake will end from
    uint256 public endBlock;
    // KK token reward per block
    ...
    event SetKKPerBlock(uint256 indexed KKPerBlock);
    ...
    event UpdatePool(uint256 indexed poolId, uint256 indexed lastRewardBlock, uint256 totalKK);
    ...
    event Claim(address indexed user, uint256 indexed poolId, uint256 KKReward);
    ...
    /**
     * @notice Set KK token address. Set basic info when deploying.
     */
    function initialize(
        IERC20 _KK,
        uint256 _startBlock,
        ...
        require(_startBlock <= _endBlock && _KKPerBlock > 0, "invalid parameters");
    ...
    /**
     * @notice Set KK token address. Can only be called by admin
     */
    function setKK(IERC20 _KK) public onlyRole(ADMIN_ROLE) {
        KK = _KK;
    ...
    /**
     * @notice Update the KK reward amount per block. Can only be called by admin.
     */
    function setKKPerBlock(uint256 _KKPerBlock) public onlyRole(ADMIN_ROLE) {
        require(_KKPerBlock > 0, "invalid parameter");

        KKPerBlock = _KKPerBlock;

        emit SetKKPerBlock(_KKPerBlock);
    }

    /**
     * @notice Add a new staking to pool. Can only be called by admin
     * DO NOT add the same staking token more than once. KK rewards will be messed up if you do
     */
    ...
    function getMultiplier(uint256 _from, uint256 _to) public view returns(uint256 multiplier) {
        ...
        bool success;
        (success, multiplier) = (_to - _from).tryMul(KKPerBlock);
        require(success, "multiplier overflow");
    }

    /**
     * @notice Get pending KK amount of user in pool
     */
    function pendingKK(uint256 _pid, address _user) external checkPid(_pid) view returns(uint256) {
        return pendingKKByBlockNumber(_pid, _user, block.number);
    }

    /**
     * @notice Get pending KK amount of user by block number in pool
     */
    function pendingKKByBlockNumber(uint256 _pid, address _user, uint256 _blockNumber) public checkPid(_pid) view returns(uint256) {
        Pool storage pool_ = pool[_pid];
        User storage user_ = user[_pid][_user];
        uint256 accKKPerST = pool_.accKKPerST;
        uint256 stSupply = pool_.stTokenAmount;

        if (_blockNumber > pool_.lastRewardBlock && stSupply != 0) {
            uint256 multiplier = getMultiplier(pool_.lastRewardBlock, _blockNumber);
            uint256 KKForPool = multiplier * pool_.poolWeight / totalPoolWeight;
            accKKPerST = accKKPerST + KKForPool * (1 ether) / stSupply;
        }

        return user_.stAmount * accKKPerST / (1 ether) - user_.finishedKK + user_.pendingKK;
    }
    ...
    function updatePool(uint256 _pid) public checkPid(_pid) {
        ...
        (bool success1, uint256 totalKK) = getMultiplier(pool_.lastRewardBlock, block.number).tryMul(pool_.poolWeight);
        require(success1, "totalKK mul poolWeight overflow");

        (success1, totalKK) = totalKK.tryDiv(totalPoolWeight);
        require(success1, "totalKK div totalPoolWeight overflow");

        uint256 stSupply = pool_.stTokenAmount;
        if (stSupply > 0) {
            (bool success2, uint256 totalKK_) = totalKK.tryMul(1 ether);
            require(success2, "totalKK mul 1 ether overflow");

            (success2, totalKK_) = totalKK_.tryDiv(stSupply);
            require(success2, "totalKK div stSupply overflow");

            (bool success3, uint256 accKKPerST) = pool_.accKKPerST.tryAdd(totalKK_);
            require(success3, "pool accKKPerST overflow");
            pool_.accKKPerST = accKKPerST;
        }

        pool_.lastRewardBlock = block.number;

        emit UpdatePool(_pid, pool_.lastRewardBlock, totalKK);
    }
    ...
    /**
     * @notice Deposit staking nativeCurrency for KK rewards
     */
    ...
    /**
     * @notice Deposit staking token for KK rewards
    ...
    function unstake(uint256 _pid, uint256 _amount) public whenNotPaused() checkPid(_pid) whenNotWithdrawPaused() {
        ...
        uint256 pendingKK_ = user_.stAmount * pool_.accKKPerST / (1 ether) - user_.finishedKK;

        if(pendingKK_ > 0) {
            user_.pendingKK = user_.pendingKK + pendingKK_;
        }
        ...
        user_.finishedKK = user_.stAmount * pool_.accKKPerST / (1 ether);
    ...
    function claim(uint256 _pid) public whenNotPaused() checkPid(_pid) whenNotClaimPaused() {
        ...
        uint256 pendingKK_ = user_.stAmount * pool_.accKKPerST / (1 ether) - user_.finishedKK + user_.pendingKK;

        if(pendingKK_ > 0) {
            user_.pendingKK = 0;
            _safeKKTransfer(msg.sender, pendingKK_);
        }

        user_.finishedKK = user_.stAmount * pool_.accKKPerST / (1 ether);

        emit Claim(msg.sender, _pid, pendingKK_);
    }
    ...
    function _deposit(uint256 _pid, uint256 _amount) internal {
        ...
        if (user_.stAmount > 0) {
            // uint256 accST = user_.stAmount.mulDiv(pool_.accKKPerST, 1 ether);
            (bool success1, uint256 accST) = user_.stAmount.tryMul(pool_.accKKPerST);
            require(success1, "user stAmount mul accKKPerST overflow");
            (success1, accST) = accST.tryDiv(1 ether);
            require(success1, "accST div 1 ether overflow");
            
            (bool success2, uint256 pendingKK_) = accST.trySub(user_.finishedKK);
            require(success2, "accST sub finishedKK overflow");

            if(pendingKK_ > 0) {
                (bool success3, uint256 _pendingKK) = user_.pendingKK.tryAdd(pendingKK_);
                require(success3, "user pendingKK overflow");
                user_.pendingKK = _pendingKK;
            }
        }
        ...
        // user_.finishedKK = user_.stAmount.mulDiv(pool_.accKKPerST, 1 ether);
        (bool success6, uint256 finishedKK) = user_.stAmount.tryMul(pool_.accKKPerST);
        require(success6, "user stAmount mul accKKPerST overflow");

        (success6, finishedKK) = finishedKK.tryDiv(1 ether);
        require(success6, "finishedKK div 1 ether overflow");

        user_.finishedKK = finishedKK;

        emit Deposit(msg.sender, _pid, _amount);
    }

    /**
     * @notice Safe KK transfer function, just in case if rounding error causes pool to not have enough KKs
     *
     * @param _to        Address to get transferred KKs
     * @param _amount    Amount of KK to be transferred
     */
    function _safeKKTransfer(address _to, uint256 _amount) internal {
        uint256 KKBal = KK.balanceOf(address(this));

        if (_amount > KKBal) {
            KK.transfer(_to, KKBal);
        } else {
            KK.transfer(_to, _amount);
        }
    }
